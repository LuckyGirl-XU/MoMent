...
class CNEN(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.time_global = nn.Sequential(
            nn.Linear(in_features=self.time_feat_dim, out_features=self.time_global_dim, bias=True),
...
#                 norm=nn.LayerNorm(self.time_global_dim)
        ...
        self.text_global = nn.Sequential(
            nn.Linear(in_features=self.node_feat_dim, out_features=self.time_global_dim, bias=True),
            nn.TransformerEncoder(
                nn.TransformerEncoderLayer(
                    d_model=self.time_global_dim,
                    nhead=8,
...
#                 norm=nn.LayerNorm(self.time_global_dim)
        ...
        self.out_layer_align = nn.Linear(self.node_feat_dim + self.time_global_dim*2, self.node_feat_dim, bias=True)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray, positive = False):
        ...
        node_time_input = torch.squeeze(self.time_encoder(timestamps=torch.from_numpy(node_interact_times[:, np.newaxis]).float().to(self.device)))
        node_time_token = self.time_global(node_time_input)
        
        node_text_input = torch.cat([self.node_raw_features[torch.from_numpy(src_node_ids)],self.node_raw_features[torch.from_numpy(dst_node_ids)]], dim =0)
        node_text_token = self.text_global(node_text_input)
        
        src_text_time_token = torch.mul(node_time_token, node_text_token[:len(node_text_token) // 2])
        dst_text_time_token = torch.mul(node_time_token, node_text_token[len(node_text_token) // 2:])
        
        node_time_token_sd = torch.cat([node_time_token, node_time_token], dim =0)
        
        src_node_embeddings = self.out_layer_align(torch.cat([src_node_embeddings,node_time_token, node_text_token[:len(node_text_token) // 2]], dim =1))
        dst_node_embeddings = self.out_layer_align(torch.cat([dst_node_embeddings,node_time_token, node_text_token[len(node_text_token) // 2:]], dim =1))
